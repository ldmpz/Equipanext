...
type GarmentType = 'camisa' | 'chaleco' | 'pantalon';
...
export default function Disenador() {
  const [garment, setGarment] = useState<GarmentType>('camisa'); // camisa, chaleco, pantalon
  const [colorName, setColorName] = useState<string>('Azul Navy');
  const [colorHex, setColorHex] = useState<string>('#1B2E3C');
  const [colorDarkHex, setColorDarkHex] = useState<string>('#12202B');
  const [reflectors, setReflectors] = useState<boolean>(false);
  const [stitching, setStitching] = useState<boolean>(false);
  const [logoType, setLogoType] = useState<string>('Bordado');
  const [logoLocation, setLogoLocation] = useState<string>('Pecho Izquierdo');
  const [quantity, setQuantity] = useState<number>(100);

  // Pricing config
  const basePrices: Record<GarmentType, { bronze: number; silver: number; gold: number }> = {
  ...
  let baseKey: 'bronze' | 'silver' | 'gold' = 'bronze';
  ...
  const garmentNameMap: Record<GarmentType, string> = {
